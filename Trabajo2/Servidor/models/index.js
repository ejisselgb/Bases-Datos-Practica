...
async getPeliculaSerieDeCategoria(nombre){
    const querySelect = `SELECT pel.titulo, pel.cartel, cat.nombre 
...
async getPeliculaDeBuscador(busqueda){
    const querySelect = `SELECT titulo FROM "Grupo 3 - T2"."PELICULAS" WHERE titulo iLIKE  '%${busqueda}%';`
    const result = await connection.query(querySelect)
    return result
},
...
async getPeliculaSeleccionada(seleccion){
    const querySelect = `SELECT * FROM "Grupo 3 - T2"."PELICULAS" WHERE id_pelicula = ${seleccion};;`
    const result = await connection.query(querySelect)
    return result
},
